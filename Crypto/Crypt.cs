    ...
    public class Crypt
    {
        ...
        static List<short> GetFile(string fileName)
        {
            ...
            if (File.Exists(fileName))
            {
                ...
                for (int i = 0; i < fi.Length; i++) bytes.Add((short)fs.ReadByte());
            }
            return bytes;
        }

        public Crypt(int passcount)
        {
            pass = Pass.FillPass(passcount, k, r);
        }

        public Crypt(string passname)
        {
            try
            {
                pass = Pass.LoadPass(passname);
            }
            catch
            { 
                throw new Exception(@"Не найден файл ключа");
            };
        }
        ...
        //{
        //    List<short> data = GetFile(filename);
        //    List<short> Arr = new List<short>();//выходной массив

        //    int ss = (int)Math.Sqrt((double)pass.S.Count);
        //    int pp = (ss / k) * n;

        //    Matrix s = new Matrix(pass.S, ss);//матрица ключа
        //    Matrix p = new Matrix(pass.P, ss);//матрица ключа
        ...
        //    Matrix dt = new Matrix(n, 1); //массивы, применяемые при кодировании
        //    Matrix a = new Matrix(k, 1);

        //    int i, j, l, t, y;// счетчики

        //    short bb;
        //    for (l = 0; l <= data.Count; l++)
        //    {
        //        if ((l % ss == 0 && l != 0) || l == data.Count)
        //        {
        //            y = 0;
        //            if (l == data.Count && l % ss != 0)
        //            {
        //                for (i = l % ss; i < ss; i++) dat.matxx[i, 0] = 0;
        //            }

        //            dat = dat.Multi(s.Triangle(), dat);//Умножение на матрицу s
        //            dat = dat.Multi(p.Triangle(), dat);//Умножение на матрицу p

        //            for (t = 0; t <= ss; t++)
        //            {
        //                if (t % k == 0 && t != 0)
        //                {
        //                    for (i = 0; i < k; i++) a.matxx[i, 0] = dt.matxx[i, 0];
        //                    for (i = 0; i < r; i++) dt.matxx[i, 0] = 0;
        //                    for (i = n - 1; i > r - 1; i--)
        //                    {
        //                        bb = Matrix.Div(dt.matxx[i, 0], g[r]);
        //                        for (j = 0; j <= r; j++) dt.matxx[i - j, 0] = (short)(dt.matxx[i - j, 0] ^ Matrix.Mullt(bb, g[r - j]));
        //                    }
        //                    for (i = 0; i < r; i++) datt.matxx[y + i, 0] = dt.matxx[i, 0];
        //                    for (i = 0; i < k; i++) datt.matxx[y + i + r, 0] = a.matxx[i, 0];
        //                    y += n;
        //                    if (t < k) dt.matxx[t % k, 0] = dat.matxx[t, 0];
        //                }
        //                else dt.matxx[t % k + r, 0] = dat.matxx[t, 0];
        //            }

        //            for (i = 0; i < n; i++) Arr.Add(datt.matxx[i, 0]);
        ...
        //    string newName = Path.Combine(Path.GetDirectoryName(filename), string.Format("cpt_{0}", Path.GetFileName(filename)));
        //    FileStream fs = new FileStream(newName, FileMode.Create, FileAccess.Write);
        //    byte[] ar = new byte[Arr.Count];
        //    for (i = 0; i < arr.Count; i++) ar[i] = (byte)Arr[i];
        //    fs.Write(ar, 0, ar.Length);
        //}

        //public void DeCrypt(string filename)
        //{
        //    List<short> arr = GetFile(filename);
        ...
        //    int a, b, i, j, q, m, d, l, ll;// счетчики
        ...
        //    {
        //        if ((a % pp == 0 && a != 0) || a == arr.Count)
        //        {
        //            if (a == arr.Count && a % pp != 0)
        //            {
        //                for (j = a % pp; j < pp; j++) datt.matxx[j, 0] = 0;
        //            }
        //            for (b = 0; b <= pp; b++)
        //            {
        //                if (b % n == 0 && b != 0)
        ...
        //                                //else recd.matxx[loc[j] - r, 0] = (short)(recd.matxx[loc[j] - r, 0] ^ vall[j]);
        //                            }
        //                        }
        //                    }
        //                    for (i = r; i < n; i++) daat.matxx[i - r, 0] = recd.matxx[i, 0];

        //                    daat = daat.Multi(p.Reverse(), daat);
        //                    daat = daat.Multi(s.Reverse(), daat);

        //                    for (i = 0; i < k; i++) data.Add(daat.matxx[i, 0]);
        //                }
        ...
        //    string newName = Path.Combine(Path.GetDirectoryName(filename), string.Format("dcpt_{0}", Path.GetFileName(filename)));
        //    FileStream fs = new FileStream(newName, FileMode.Create, FileAccess.Write);
        //    byte[] ar = new byte[data.Count];
        //    for (i = 0; i < data.Count; i++) ar[i] = (byte)data[i];
        //    fs.Write(ar, 0, ar.Length);
        //}
        ...
        public void EnCrypt(string filename)
        {
            ...
            for (i = 0; i < Arr.Count; i++) ar[i] = (byte)Arr[i];
            fs.Write(ar, 0, ar.Length);
            MessageBox.Show(string.Format("Зашифрованное сообщение сохранено в {0}", newName));
        }
        ...
        public void DeCrypt(string filename)
        {
            ...
            for (i = 0; i < data.Count; i++) ar[i] = (byte)data[i];
            fs.Write(ar, 0, ar.Length);
            MessageBox.Show(string.Format("Расшифрованное сообщение сохранено в {0}", newName));
        }


        private List<short> EnCode(Matrix dt)
        {
            int i, j;
            short bb;
        ...
        private Matrix DeCode(Matrix recd)
        {
            int i, j, m, l, ll, q, d;
            bool syn_error = false;//признак ошибки
        ...
        private List<short> HoloCode(List<short> Arr)
        {
            int i, j, l;
            List<short> arr = new List<short>();//псевдоголографическое кодирование
            if (pass.First.Count > Arr.Count)
        ...
        private List<short> HoloDeCode(List<short> arr)
        {
            int i, j, m, d, l, ll;
            if (pass.First.Count > arr.Count)
            {
